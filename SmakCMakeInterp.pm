...
sub generate_makefiles {
    ...
    for my $name (sort keys %{$state->{targets}}) {
        ...
        next if $name =~ /::/;
        # Skip targets with no sources unless they're custom, OR they link
        # other targets (e.g. an executable/library built purely from OBJECT
        # libraries, like yosys and libyosys — all code comes from the linked
        # component object libs, the target itself has no direct sources).
        next if !@{$t->{sources}} && !@{$t->{link_libraries} // []}
                && $t->{type} ne 'custom';
...
sub _write_makefile2 {
    ...
    # Collect real build targets: anything with sources, or a link-only
    # target that links other targets (OBJECT-library executables/libraries).
    my @real = grep {
        my $t = $state->{targets}{$_};
        $t->{type} ne 'custom'
            && !($t->{libtype} && $t->{libtype} =~ /^(interface|imported|alias)$/)
            && $_ !~ /::/
            && (@{$t->{sources}} > 0 || @{$t->{link_libraries} // []} > 0)
...
# Absolute paths to an OBJECT library's compiled object files. Must match the
# obj layout SmakCMake compiles to: <binary_dir>/CMakeFiles/<name>.dir/<base>.o
sub _object_files_of {
    my ($lt, $libname) = @_;
    my $bin = $lt->{binary_dir} // '';
    my (%seen, @objs);
    for my $s (@{$lt->{sources} // []}) {
        (my $b = $s) =~ s{^.*/}{};
        my $o = "$bin/CMakeFiles/$libname.dir/$b.o";
        push @objs, $o unless $seen{$o}++;
    }
    return @objs;
}
...
sub _resolve_link_libraries {
    ...
    while (@queue) {
        ...
        if ($lt) {
            ...
            # OBJECT library: it produces no archive — its object files are
            # linked directly into the consumer. Emit each .o path.
            if (($lt->{libtype} // '') eq 'object') {
                push @out, _object_files_of($lt, $lib);
                next;
            }
