...
sub _write_cmake_cache {
    my ($state) = @_;
    my $build_dir = $state->{build_dir};
    # Honor -D/CMAKE_*_COMPILER overrides (e.g. mpicc/mpicxx for an MPI build)
    # instead of hardcoding the host defaults: smak reads this cache to pick the
    # compiler, so a hardcoded value silently ignores the user's -D selection.
    my $rs = $state->{root_scope} || {};
    my $pick = sub {
        my ($name, $dflt) = @_;
        return $rs->{vars}{$name} // $rs->{cache}{$name} // $dflt;
    };
    ...
    # smak reads these back to pick the compile-command compiler, so apply the
    # same non-executable-path fallback used for flags.make/link.txt — otherwise
    # a bogus CMAKE_<lang>_COMPILER (e.g. BUILD_STATS' wrapper) lands here and
    # every compile exits 127.
    print $fh "CMAKE_C_COMPILER:FILEPATH=" . _usable_compiler($pick->('CMAKE_C_COMPILER', '/usr/bin/cc'), '/usr/bin/cc', 'C') . "\n";
    print $fh "CMAKE_CXX_COMPILER:FILEPATH=" . _usable_compiler($pick->('CMAKE_CXX_COMPILER', '/usr/bin/c++'), '/usr/bin/c++', 'CXX') . "\n";
    print $fh "CMAKE_Fortran_COMPILER:FILEPATH=" . _usable_compiler($pick->('CMAKE_Fortran_COMPILER', '/usr/bin/gfortran'), '/usr/bin/gfortran', 'Fortran') . "\n";
    print $fh "CMAKE_AR:FILEPATH=" . $pick->('CMAKE_AR', '/usr/bin/ar') . "\n";
    print $fh "CMAKE_RANLIB:FILEPATH=" . $pick->('CMAKE_RANLIB', '/usr/bin/ranlib') . "\n";
    close($fh);
}
...
# Return $cand if it's a usable compiler, else $dflt. Guards against an explicit
# CMAKE_<lang>_COMPILER PATH that doesn't exist / isn't executable — e.g. Trilinos
# BUILD_STATS points CMAKE_CXX_COMPILER at a generated build_stat_cxx_wrapper.sh
# that smak never emits, which otherwise makes every compile exit 127. Bare
# command names (mpicxx, g++-14) are trusted: they resolve via PATH at build time.
my %_compiler_warned;
sub _usable_compiler {
    my ($cand, $dflt, $label) = @_;
    return $dflt unless defined $cand && $cand ne '';
    if ($cand =~ m{/} && !(-f $cand && -x $cand) && $cand ne $dflt) {
        # _compiler_for_lang runs per-target — warn once per bad compiler, not
        # once per target, so one bogus path doesn't flood a large build.
        warn "SmakCMakeInterp: CMAKE_${label}_COMPILER='$cand' is not an " .
             "executable file; falling back to $dflt\n"
            unless $_compiler_warned{"$label\t$cand"}++;
        return $dflt;
    }
    return $cand;
}
...
sub _compiler_for_lang {
    my ($lang, $state) = @_;
    # Honor a -D/CMAKE_<lang>_COMPILER override (mpicc/mpicxx, a specific gcc
    # version, etc.) from the root scope/cache; fall back to host defaults.
    # Used for BOTH the compile rules (flags.make) and the link command
    # (link.txt), so e.g. mpicxx links pull in -lmpi automatically.
    my %dflt = (
        C       => '/usr/bin/cc',
        CXX     => '/usr/bin/c++',
    ...
    if ($state && $state->{root_scope}) {
        my $rs = $state->{root_scope};
        my $v = $rs->{vars}{"CMAKE_${lang}_COMPILER"}
             // $rs->{cache}{"CMAKE_${lang}_COMPILER"};
        return _usable_compiler($v, ($dflt{$lang} // '/usr/bin/cc'), $lang)
            if defined $v && $v ne '';
    }
    return $dflt{$lang} // '/usr/bin/cc';
}
