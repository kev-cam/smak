#!/bin/bash
# Test building ghdl project with smak
# This is a regression test - ghdl used to build successfully

echo "Testing ghdl build with smak..."
echo ""

cd "$(dirname "$0")"
FAILED=0

# Check if ghdl directory exists
GHDL_DIR="/usr/local/src/ghdl"
if [ ! -d "$GHDL_DIR" ]; then
    echo "  ✗ FAIL: ghdl directory not found at $GHDL_DIR"
    exit 1
fi

echo "Test: Build ghdl with smak"
cd "$GHDL_DIR" || exit 1
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "Note: This is a regression - ghdl used to build successfully with smak"
    exit 1
fi
