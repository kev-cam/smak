...
# This allows the recursive-make regex (which requires -C or -f) to match
# commands that use the cd-then-make pattern.
sub normalize_cd_make {
    my ($cmd) = @_;
    if ($cmd =~ /^cd\s+(\S+)\s+&&\s+(.*?\b(?:make|smak)\b.*)$/s) {
        my ($dir, $make_cmd) = ($1, $2);
        $dir =~ s/^['"]|['"]$//g;
            ...
            if ($child_data) {
                for my $tgt (keys %$child_data) {
                    ...
                    $capture_targets->{$full_tgt} = {
                        ...
                        rule => $info->{rule} || '',
                        expanded_rule => $info->{expanded_rule} || '',
...
sub build_target {
    ...
    # Capture target info for btree if capture mode is active
    if ($capture_targets) {
        use Cwd 'getcwd';
        my $cwd = getcwd();
    ...
    if ($rule && $rule =~ /\S/) {
        ...
        if ($capture_targets && exists $capture_targets->{$target}) {
            $capture_targets->{$target}{expanded_rule} = $expanded;
        }
        ...
        for my $cmd_line (split /\n/, $expanded) {
            ...
            if (($dry_run_mode || !$job_server_socket) && $clean_cmd =~ /&&/) {
                ...
                for my $part (@parts) {
                    ...
                    # Check if it's a built-in we can handle (rm, cd, make/smak)
                    # Note: 'cd' is handled specially - it's just skipped in execution since
                    # the directory change is incorporated into the -C option of make/smak
                    unless ($check_part =~ /^\s*(?:rm|cd)\b/ || $check_part =~ /\b(make|smak)\s/ || $check_part =~ m{/smak(?:\s|$)}) {
                ...
                if ($can_handle_all) {
                    ...
                    # Print the original command first (like make -n does)
                    ...
                    for my $i (0 .. $#parts) {
                        ...
                        # Skip cd commands - directory change is handled via -C option
                        # by normalize_cd_make() earlier in the pipeline
            ...
            if ($clean_cmd =~ /^rm\s+(.+)$/ && $clean_cmd !~ /\s(?:&&|;)\s/) {
                ...
                # In dry-run mode, print even if @ prefix was used (like make -n)
                unless ($silent_mode || (!$dry_run_mode && $silent)) {
                    print "$display_cmd\n";
                }
            ...
            if ($normalized_cmd =~ /^(?:[\w\/.-]*\/)?(?:make|smak)(?:\.pl)?(?:\s|$)/) {
                ...
                if ($sub_directory) {
                    ...
                    # In sequential mode without job server: fall back to external execution
                    if ($sub_vars_ref && %$sub_vars_ref) {
                        my $has_backticks = 0;
                        for my $var (keys %$sub_vars_ref) {
                            if ($sub_vars_ref->{$var} =~ /`/) {
                        ...
                        if ($has_backticks) {
                            if ($dry_run_mode) {
                                # Dry-run mode: expand backticks via shell so we can show expanded commands
                                warn "DEBUG[" . __LINE__ . "]: Dry-run mode - expanding backticks via shell\n" if $ENV{SMAK_DEBUG};
                                for my $var (keys %$sub_vars_ref) {
                                    if ($sub_vars_ref->{$var} =~ /`/) {
                                        my $val = $sub_vars_ref->{$var};
                                        ...
                                        chomp $expanded if defined $expanded;
                                        $sub_vars_ref->{$var} = $expanded // $val;
                                        warn "DEBUG[" . __LINE__ . "]: Expanded $var: '$val' -> '$expanded'\n" if $ENV{SMAK_DEBUG};
                                    }
                                }
                            } else {
                                # Sequential mode: fall back to external execution
                                warn "DEBUG[" . __LINE__ . "]: Variable contains backticks, falling back to external execution\n" if $ENV{SMAK_DEBUG};
                                goto EXECUTE_EXTERNAL_COMMAND;
                            }
                        }
                    }
...
# dependencies/rules starting from a root target. Used by btree and job server.
# Returns: (\%all_targets, \%target_info)
#   all_targets: target => [deps]
#   target_info: target => { rule, exec_dir, siblings, compound_parent }
sub collect_target_graph {
    my ($root_target) = @_;
    my %all_targets;
    my %target_info;
    my %visited;
    ...
    $walk = sub {
        ...
        # Expand variables in deps (same as build_target: format_output + expand_vars)
        @deps = map {
            my $d = $_;
            $d = format_output($d);
            $d = expand_vars($d);
            if ($d =~ /\s/) { split /\s+/, $d; } else { $d; }
        } @deps;
        @deps = grep { defined $_ && $_ ne '' && /\S/ && !/dirstamp$/ && !/\.deps\// && !m{^/usr/} } @deps;
...
# Run the built-in worker protocol test suite.
# Accepts a connected worker socket and exercises the full EXTERNAL_CMDS protocol.
...
sub run_test_worker {
    ...
    # Helper: read lines from worker until READY, collecting them
    my $read_until_ready = sub {
        my @lines;
        while (1) {
            my $line = <$worker>;
    ...
    # Helper: send a task and collect response
    my $send_task = sub {
        my (%opts) = @_;
        $task_id++;
        my $dir = $opts{dir} // '/tmp';
        my @cmds = @{$opts{cmds} // ['true']};
        my @builtins = @{$opts{builtins} // []};
        my $dry = $opts{dry} ? '_DRY' : '';

        print $worker "TASK $task_id\n";
        print $worker "DIR $dir\n";
        print $worker "EXTERNAL_CMDS$dry " . scalar(@cmds) . "\n";
        print $worker "$_\n" for @cmds;
    ...
    print "Worker Protocol Test Suite\n";
    print "=" x 40 . "\n";
    ...
    # --- Test 1: READY handshake ---
    {
        ...
        $report->(defined $line && $line eq 'READY',
                  'READY handshake',
                  "expected READY, got: " . ($line // 'EOF'));
    }

    # --- Test 2: ENV exchange ---
    {
        print $worker "ENV_START\n";
        print $worker "ENV SMAK_TEST_VAR=hello_from_test\n";
        print $worker "ENV_END\n";
        $worker->flush();
        # Worker silently absorbs ENV - no response expected.
        # We verify it worked by using the variable in a later test.
        $report->(1, 'ENV exchange', '');
    }

    # --- Test 3: Simple task (true) ---
    {
        my ($ok, @lines) = $send_task->(cmds => ['true']);
        my $has_start = grep { /^TASK_START $task_id$/ } @lines;
        my $has_end = grep { /^TASK_END $task_id 0$/ } @lines;
        my $has_ready = grep { $_ eq 'READY' } @lines;
        $report->($ok && $has_start && $has_end && $has_ready,
                  'Simple task (true)',
                  "responses: " . join(', ', @lines));
    }

    # --- Test 4: Output capture (echo) ---
    {
        my ($ok, @lines) = $send_task->(cmds => ['echo hello_world']);
        my @outputs = grep { /^OUTPUT / } @lines;
        my $has_hello = grep { /hello_world/ } @outputs;
        my $has_end = grep { /^TASK_END $task_id 0$/ } @lines;
        $report->($ok && $has_hello && $has_end,
                  'Output capture (echo)',
                  "outputs: " . join(', ', @outputs));
    }

    # --- Test 5: Multi-command task ---
    {
        my ($ok, @lines) = $send_task->(cmds => ['echo first', 'echo second']);
        my @outputs = map { /^OUTPUT (.*)/ ? $1 : () } @lines;
        my $has_first = grep { /first/ } @outputs;
        my $has_second = grep { /second/ } @outputs;
        $report->($ok && $has_first && $has_second,
                  'Multi-command task',
                  "outputs: " . join(', ', @outputs));
    }

    # --- Test 6: Trailing builtins ---
    {
        my $test_dir = "/tmp/smak-test-worker-$$";
        # First ensure clean state
        system("rm -rf $test_dir");
        my ($ok, @lines) = $send_task->(
            cmds => ['true'],
            builtins => ["mkdir -p $test_dir"],
        );
        my $dir_exists = -d $test_dir;
        system("rm -rf $test_dir");
        my $has_end = grep { /^TASK_END $task_id 0$/ } @lines;
        $report->($ok && $dir_exists && $has_end,
                  'Trailing builtins (mkdir)',
                  "dir_exists=$dir_exists, responses: " . join(', ', @lines));
    }

    # --- Test 7: Failure handling ---
    {
        my ($ok, @lines) = $send_task->(cmds => ['false']);
        my ($end_line) = grep { /^TASK_END $task_id / } @lines;
        my $exit_nonzero = 0;
        if ($end_line && $end_line =~ /^TASK_END \d+ (\d+)$/) {
            $exit_nonzero = ($1 != 0);
        }
        $report->($ok && $exit_nonzero,
                  'Failure handling (false)',
                  "end_line: " . ($end_line // 'none'));
    }

    # --- Test 8: Directory handling ---
    {
        my ($ok, @lines) = $send_task->(dir => '/tmp', cmds => ['pwd']);
        my @outputs = map { /^OUTPUT (.*)/ ? $1 : () } @lines;
        # pwd may resolve symlinks, so check for /tmp or /private/tmp
        my $in_tmp = grep { m{/tmp} } @outputs;
        $report->($ok && $in_tmp,
                  'Directory handling (pwd in /tmp)',
                  "outputs: " . join(', ', @outputs));
    }

    # --- Test 9: ENV variable propagation ---
    {
        my ($ok, @lines) = $send_task->(cmds => ['sh -c "echo $SMAK_TEST_VAR"']);
        my @outputs = map { /^OUTPUT (.*)/ ? $1 : () } @lines;
        my $has_val = grep { /hello_from_test/ } @outputs;
        $report->($ok && $has_val,
    ...
    # --- Test 10: SHUTDOWN ---
    {
        ...
        $worker->flush();
        # Worker should disconnect cleanly
        my $line = <$worker>;
        # EOF (undef) or no more data = clean shutdown
        $report->(!defined $line,
                  'SHUTDOWN (clean disconnect)',
                  "got: " . ($line // 'EOF (correct)'));
    }
...
sub unified_cli {
    ...
    if ($detached) {
        ...
        # Tell job-server to linger (don't exit on disconnect)
        if ($socket) {
            print $socket "DETACH\n";
            $socket->flush();
        }
	...
	$ret = "stop";
        ...
        # EOF on input (script ended, pipe closed) - shut down cleanly
        if ($socket) {
            print $socket "SHUTDOWN\n";
            my $ack = <$socket>;
        }
...
sub cmd_dry_run {
    ...
    if ($pid == 0) {
        # Child process - run dry-run via build_target (respects $dry_run_mode) and exit
...
sub cmd_btree {
    ...
    # Collect targets by running a dry-run with capture mode
    # This reuses build_target's full logic (recursive make, backtick expansion, etc.)
    my %captured_info;
    {
        my $capture_file = "/tmp/btree_capture_$$.dat";
    ...
    # Build dependency graph from captured data
    my %all_targets;
    my %target_info;
    for my $tgt (keys %captured_info) {
        my $info = $captured_info{$tgt};
        $all_targets{$tgt} = $info->{deps} || [];
        $target_info{$tgt} = {
    ...
    print "Collected " . scalar(keys %all_targets) . " targets from dry-run\n";
...
";
    ...
    sub split_command_parts {
        ...
        # First, split on newlines - each recipe line is a separate command
        my @lines = split(/\n/, $command);
        my @external_parts;
        my @trailing_builtins;
        my $in_trailing_builtins = 0;

        # Process each line (from end to start to detect trailing builtins)
        for my $i (reverse 0 .. $#lines) {
            ...
            if ($keep_together) {
                ...
                # Single command (no &&)
                if ($clean_line =~ /^\s*(rm|mkdir|mv|cp|echo|true|false|:|touch)\b/) {
                    if (!$in_trailing_builtins && $i == $#lines) {
                        unshift @trailing_builtins, $line;
    ...
    # Assert that a command has no unexpanded makefile variables.
    # Called before sending commands to workers - any unexpanded variables
    # indicate a bug in the expansion pipeline.
    sub assert_no_unexpanded_vars {
        my ($cmd, $target, $context) = @_;
        return unless defined $cmd && $cmd =~ /\S/;
        $context //= 'dispatch';

        # $MV{VAR} is smak's internal pre-expansion format - always a bug if present
        if ($cmd =~ /\$MV\{([^}]+)\}/) {
            die "ASSERTION FAILED [$context]: Unexpanded \$MV{$1} in command for target '$target':\n$cmd\n";
        }

        # Check for make automatic variables that should have been expanded:
        # $@ (target), $< (first prereq), $^ (all prereqs), $* (stem)
        # But $@ is valid shell ("$@" = all args), so only flag bare occurrences
        # that look like make usage (not inside double-quotes preceded by \)
        for my $var (qw($@ $< $^ $*)) {
            my $escaped = quotemeta($var);
            # Match the variable NOT preceded by \ and NOT inside $(...) shell syntax
            if ($cmd =~ /(?<!\\)${escaped}(?![{(a-zA-Z0-9_])/) {
                # $@ in shell is common ("$@"), so skip it
                next if $var eq '$@';
                warn "WARNING [$context]: Possible unexpanded automatic variable '$var' in command for target '$target':\n$cmd\n";
            }
        }

        # Check for $(ALLCAPS_VAR) patterns that look like makefile variables
        # Skip: $(shell ...), $(wildcard ...), and lowercase/mixed-case (likely shell subcommands)
        while ($cmd =~ /\$\(([A-Z][A-Z0-9_]*)\)/g) {
            my $var = $1;
            # These are common make functions, not variables - skip them
            next if $var =~ /^(SHELL|MAKE|MAKEFLAGS|MFLAGS|CURDIR|MAKELEVEL)$/;
            warn "WARNING [$context]: Possible unexpanded makefile variable '\$($var)' in command for target '$target':\n$cmd\n";
        }
    }
    ...
    sub check_child_completion {
        ...
        # Check if all child's jobs are done AND child has finished submitting (CHILD_DONE received)
        # Without the CHILD_DONE check, we'd race: a fast worker could complete the first job
        # before the remaining SUBMIT_JOBs are read from the socket, causing premature CHILD_COMPLETE.
        if (!keys %{$child_job_targets{$child_sock}} && $child_all_submitted{$child_sock}) {
    ...
    sub dispatch_job_to_worker {
        ...
        # If external_commands not set, generate from command
        my @ext_cmds;
        my @builtins;
        if ($job->{external_commands}) {
            @ext_cmds = @{$job->{external_commands}};
            @builtins = $job->{trailing_builtins} ? @{$job->{trailing_builtins}} : ();
        } elsif ($job->{command} && $job->{command} =~ /\S/) {
            # Generate external_commands from command (for jobs created during expansion)
            my ($ext_ref, $builtin_ref) = split_command_parts($job->{command});
            @ext_cmds = @$ext_ref;
            @builtins = @$builtin_ref;
        }

        # Assert no unexpanded variables before sending to worker
        for my $cmd (@ext_cmds, @builtins) {
            assert_no_unexpanded_vars($cmd, $target, 'dispatch');
        }
        ...
        for my $cmd (@ext_cmds) {
            if ($ENV{SMAK_DEBUG}) {
                my $debug_cmd = $cmd;
                $debug_cmd =~ s/\n/\\n/g;
                print STDERR "DEBUG DISPATCH: Sending cmd to worker: [$debug_cmd]\n";
            }
            print $worker "$cmd\n";
        }
	...
	my $j = 0;
		...
		while ($i <= $#job_queue) {
            ...
            if (defined $peek_cmd && $peek_cmd =~ /\S/) {
                ...
                if (!$no_builtins && is_builtin_command($peek_cmd)) {
                    ...
                    # BUT: keep 'cd /path && cmd' together (cd only affects its subprocess)
                    my @cmd_parts;
	    ...
	    $j++;
            ...
            # Send external commands using EXTERNAL_CMDS protocol
            # (properly handles cd && make as a single command unit)
            my @ext_cmds;
            my @builtins;
            if ($job->{external_commands}) {
                @ext_cmds = @{$job->{external_commands}};
            ...
            print $ready_worker "TRAILING_BUILTINS " . scalar(@builtins) . "\n";
            for my $cmd (@builtins) {
                print $ready_worker "$cmd\n";
            }
            $ready_worker->flush();
    ...
    while (1) {
        ...
        # If idle and master disconnected (not via explicit detach), exit immediately
        # The idle timeout only applies when the server was explicitly detached (SIGHUP/detach cmd)
        if ($is_idle && !defined($master_socket) && !$explicitly_detached) {
                ...
                while (my $line = <$worker>) {
                    ...
                    if ($line eq 'READY') {
                        ...
                        if ($exit_code == 0 && $job->{target}) {
                            ...
                            # CMake-style phony targets: *.dir/all, *.dir/depend, *.dir/build, */all, */clean
                            my $is_cmake_phony = $target =~ m{/(all|clean|depend|build|fast|rule)$};

                            # touch_nocreate commands don't create files, just touch existing ones
                            my $is_touch_nocreate = $job->{command} && $job->{command} =~ /touch_nocreate/;

                            # Check if target is explicitly marked as phony in Makefile
                            my $is_phony = $is_common_phony || $is_cmake_phony || $is_touch_nocreate || exists $pseudo_deps{"$makefile\t$target"};
                        ...
                        if ($task_handled_successfully || ($exit_code == 0 && $job->{target} && $completed_targets{$job->{target}})) {
                            ...
                            if (ASSERTIONS_ENABLED && $ENV{SMAK_DEBUG}) {
                                ...
                                # 4. Check for CMake-style phony targets (*.dir/all, */build, */depend, etc.)
                                if (!$is_phony_target && $target =~ m{/(all|clean|depend|build|fast|rule)$}) {
                                    $is_phony_target = 1;
                                }

                                # 5. Check pseudo_rule as fallback
                                if (!$is_phony_target) {
                                    for my $key (keys %pseudo_rule) {
                                        if ($key =~ /\t\Q$target\E$/) {
                                            $is_phony_target = 1;
        ...
        for my $socket (@ready) {
            if ($socket == $master_server) {
                # New connection on master port - could be master or child smak
                my $new_conn = $master_server->accept();
                if ($new_conn) {
                    $new_conn->autoflush(1);

                    # Read first line to determine connection type
                    my $first_line = <$new_conn>;
                    ...
                    if ($first_line eq 'CHILD_CONNECT') {
                        # Child smak connecting - add to child sockets, keep master intact
                        print STDERR "Child smak connecting\n" if $ENV{SMAK_DEBUG};
                        $select->add($new_conn);
                        push @child_sockets, $new_conn;
                        print $new_conn "CHILD_READY\n";
                        $new_conn->flush();
                        print STDERR "Child smak ready, sent CHILD_READY\n" if $ENV{SMAK_DEBUG};
                    } elsif ($first_line eq 'SHUTDOWN') {
                        # Remote shutdown request (from kill_old_js or similar)
                        print STDERR "Remote SHUTDOWN request received\n" if $ENV{SMAK_DEBUG};
                        # Send ack, then shut down
                        for my $worker (@workers) {
                            print $worker "SHUTDOWN\n";
                        }
                        for my $worker (@workers) {
              ...
              MASTER_READ:
                ...
                if ($line eq 'SHUTDOWN') {
                    ...
                    # Parse: makefile directory [VAR=value...] target1 target2 ...
                    my @parts = split(/\s+/, $build_args);
                    my $sub_makefile = shift @parts;
                    my $dir = shift @parts;

                    # Separate variable assignments (contain '=') from targets
                    my @var_assignments;
                    my @targets;
                    for my $part (@parts) {
                        if ($part =~ /^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/) {
                            push @var_assignments, [$1, $2];
                        } else {
                            push @targets, $part;
                        }
                    }
                    ...
                    # Save and set command-line variable overrides
                    my %saved_vars;
                    for my $assignment (@var_assignments) {
                        my ($var, $val) = @$assignment;
                        # Decode base64 if needed
                        if ($val =~ /^BASE64:(.+)$/) {
                            require MIME::Base64;
                            $val = MIME::Base64::decode_base64($1);
                        }
                        $saved_vars{$var} = $MV{$var};
                        $MV{$var} = $val;
                        warn "BUILD: Set variable $var\n" if $ENV{SMAK_DEBUG};
                    }
		    ...
		    my $w_count = 1 + $#workers;
            ...
            } elsif (grep { $_ == $socket } @child_sockets) {
                # Message from a child smak - drain ALL available messages
                # (Perl's <> reads kernel buffer into userspace; select() only sees kernel buffer,
                # so we must loop to consume all buffered data or messages get stranded)
                $socket->blocking(0);
              ...
              CHILD_READ:
                ...
                if ($line eq 'SUBMIT_JOB') {
                    ...
                    my @deps;
                    my $deps_header = <$socket>;
                    chomp $deps_header if defined $deps_header;
                    if (defined $deps_header && $deps_header =~ /^DEPS (\d+)$/) {
                        my $count = $1;
                        for (1..$count) {
                            my $dep_line = <$socket>;
                            chomp $dep_line if defined $dep_line;
                            push @deps, $dep_line if defined $dep_line;
                        }
                    }
                    ...
                    # Mark that this child has finished submitting - check_child_completion
                    # can now safely send CHILD_COMPLETE when all jobs are done
                    $child_all_submitted{$socket} = 1;
                    ...
                    if ($job_count == 0 || !exists $child_job_targets{$socket} ||
                        ...
                        print STDERR "Child has no pending jobs, sending CHILD_COMPLETE $child_exit\n" if $ENV{SMAK_DEBUG};
                ...
                if (@child_jobs) {
                    ...
                    for my $job (@child_jobs) {
                        $child_targets{$job->{target}} = 1;
                    }

                    # Mark source file deps as completed (they aren't child jobs and exist on disk)
                    for my $job (@child_jobs) {
                        for my $dep (@{$job->{deps}}) {
                    ...
                    # Propagate: if any dep is being rebuilt, this target needs rebuild too
                    my $changed = 1;
                    while ($changed) {
                        $changed = 0;
                        for my $job (@child_jobs) {
                    ...
                    my @to_remove;
                    for my $job (@child_jobs) {
                        unless ($needs_build{$job->{target}}) {
                            print STDERR "Child job '$job->{target}' is up-to-date, skipping\n" if $ENV{SMAK_DEBUG};
                    ...
                    # Remove up-to-date jobs from queues
                    if (@to_remove) {
                        my %remove_set = map { $_ => 1 } @to_remove;
                        @job_queue = grep { !($remove_set{$_->{target}}) } @job_queue;
                        for my $layer_ref (@job_layers) {
                            next unless $layer_ref;
                            @$layer_ref = grep { !($remove_set{$_->{target}}) } @$layer_ref;
                        }
                ...
                # Advance dispatch layer past empty layers (up-to-date removal may have cleared lower layers)
                while ($current_dispatch_layer <= $max_dispatch_layer) {
                    my $layer_ref = $job_layers[$current_dispatch_layer];
			...
			send_env($socket);
                    ...
                    if ($exit_code == 0 && $job->{target}) {
                        ...
                        # Only verify file existence for targets that look like real files
                        # Skip verification in dry-run mode since files aren't actually created
                        my $should_verify = $looks_like_file && !$is_common_phony && !$is_cmake_phony && !$is_touch_nocreate && !$dry_run_mode;
