...
while (my $line = <$socket>) {
	...
	get_task($1,$env_done);
        ...
        } # End of if (0) block - disabled in dry-run mode

        # Assert that we should be using built-in optimizations (for testing)
        if (0 && $ENV{SMAK_ASSERT_NO_SPAWN} && @recursive_calls > 0) {  # Disabled in dry-run
        ...
        # DRY-RUN MODE: Just print command instead of executing
        # Declare variables for compatibility (even though not used in dry-run)
        my $exit_code = 0;
        my @errors;
        my @warnings;
        my $max_retries = 3;
        ...
        if (0) {
            # This block is never executed in dry-run mode
        ...
        # Restore directory (not needed in dry-run mode)
        # chdir($old_dir);
    } else {
